"""
Advanced Responsive Web Application for Phishing URL Detection
...
class AdvancedURLAnalyzer:
    def __init__(self):
        self.threat_categories = {
            'phishing': ['paypal', 'amazon', 'google', 'microsoft', 'apple', 'facebook', 'instagram', 'twitter'],
            'banking': ['bank', 'banking', 'finance', 'credit', 'loan', 'mortgage', 'investment'],
            'crypto': ['bitcoin', 'ethereum', 'crypto', 'wallet', 'exchange', 'trading', 'coin'],
            'social': ['facebook', 'instagram', 'twitter', 'linkedin', 'tiktok', 'snapchat'],
            'ecommerce': ['amazon', 'ebay', 'shop', 'store', 'buy', 'sale', 'discount'],
            'security': ['secure', 'verify', 'update', 'confirm', 'suspend', 'urgent', 'alert']
        }
        ...
        self.suspicious_tlds = ['.tk', '.ml', '.ga', '.cf', '.gq', '.pw', '.top', '.click', '.download']
...
async def download_report(analysis_id: str):
    ...
    report_path = f"reports/security_report_{analysis_id}.pdf"
    
    if os.path.exists(report_path):
        return FileResponse(
            report_path,
            media_type="application/pdf",
            filename=f"URL_Security_Report_{analysis_id[:8]}.pdf"
        )
    else:
...
async def generate_detailed_report(analysis_id: str, analysis_data: Dict):
    ...
    try:
        ...
        # Title
        story.append(Paragraph("🛡️ URL Security Analysis Report", title_style))
        story.append(Spacer(1, 20))
        
        # Executive Summary
        story.append(Paragraph("Executive Summary", heading_style))
        
        risk_color = colors.red if analysis_data.get("risk_score", 0) > 60 else colors.orange if analysis_data.get("risk_score", 0) > 30 else colors.green
        ...
        summary_data = [
            ...
            ["Confidence", f"{analysis_data.get('confidence_score', 0)*100:.1f}%"],
        ...
        # Threats Detected
        if analysis_data.get("threats_detected"):
            story.append(Paragraph("🚨 Threats Detected", heading_style))
            for i, threat in enumerate(analysis_data["threats_detected"], 1):
                story.append(Paragraph(f"{i}. {threat}", styles['Normal']))
        ...
        for key, value in indicators.items():
            if isinstance(value, (int, float, bool, str)):
                assessment = "✅ Good" if (
                    (key == "has_https" and value) or
                    (key == "url_length" and value < 100) or
                    (key == "subdomain_count" and value <= 2)
                ) else "⚠️ Check" if (
                    (key == "url_length" and 100 <= value < 200) or
                    (key == "subdomain_count" and value == 3)
                ) else "❌ Risk"
        ...
        tech_details = analysis_data.get("technical_details", {})
        
        for section, data in tech_details.items():
            if isinstance(data, dict):
                story.append(Paragraph(f"<b>{section.replace('_', ' ').title()}</b>", styles['Normal']))
        ...
        story.append(Paragraph(
            f"Report generated on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}<br/>"
            f"Analysis ID: {analysis_id}<br/>"
            f"SecureURL Guardian v3.0 - Advanced Threat Detection System",
            styles['Normal']
        ))
